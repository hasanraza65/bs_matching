...
const InvoicePaymentModalInner: React.FC<InvoicePaymentModalProps> = ({ isOpen, onClose, invoice, onSuccess }) => {
          ...
          <>
            ...
            <div className="text-center mb-8">
              <h3 className="text-2xl font-display font-bold text-slate-900 mb-2 uppercase tracking-tight">
                {isProcessing ? t.processing : "Pay Invoice"}
              </h3>
              <p className="text-slate-500 text-sm">
                Invoice <span className="font-bold text-slate-700">{invoice.invoice_num}</span>
              </p>
            </div>

            <div className="bg-slate-50 rounded-2xl p-5 mb-8 border border-slate-100 flex justify-between items-center relative z-10">
            ...
            <div className={`space-y-4 mb-8 transition-opacity duration-300 relative z-10 ${isProcessing ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
            ...
            <div className={`px-2 relative z-10 transition-opacity duration-300 ${isProcessing ? 'opacity-0' : 'opacity-100'}`}>
              <SlideToAccept
                ...
                label={t.payButton.replace('{amount}', `${parseFloat(invoice.amount).toFixed(2)} €`)}
              />
            </div>
          </>
        ) : (
